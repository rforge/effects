...
\cite{VenablesRipley02} provide a penalized quasi-likelihood function for fitting linear and generalized linear mixed models in their \code{MASS} package. The following function added to \code{effects} in March 2020 allows these fitted models to be used with the \code{effects} package
<<>>=
