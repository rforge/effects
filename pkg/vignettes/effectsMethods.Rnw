...
\end{description}
\emph{Since the values of all the arguments in \code{sources} are default values for the \code{gls} method, there is no need to have written the \code{Effect.gls} method, as the default method would work.} 
