...
\title{Predictor Effects Gallery}
...
\begin{abstract}
Predictor effect displays visualize the response surface of complex regression models by averaging and conditioning, producing a sequence of 2D line graphs, one graph or set of graphs for each predictor in the regression problem \citep{fw19b, fw19}.  In this vignette we give examples of the use of the \pkg{effects} package in \R{} for drawing effects plots, including many of the optional arguments.
\end{abstract}
