...
For a second example, we turn fit a linear model with an interaction, modelling national \vn{infantMortality} rate (infant deaths per 1000 live births) as a function of \vn{ppgdp}, per person GDP (in U.S.~dollars), and country \vn{group} (OECD nation, African nations, and other nations), to the \code{UN} data set in the \pkg{carData} package. The data are for roughly 200 nations of the world and are from approximately 2009 to 2011:
