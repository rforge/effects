...
Interpretation of GLM predictor effect plots in link scale is similar to predictor effect plots for linear models, and all the modifications previously described can be used for these plots.  Because the default is \code{type="rescale"}, the vertical axis is in linear predictor scale, which is the log-odds for logit for this logistic regression example, but the vertical axis labels are in probability scale, so the tick-marks are not equally spaced.
