...
\subsection{\ar{focal.levels} and \ar{xlevels}: Options for the Focal Predictor and Predictors in the Conditioning Group}
...
In this example using three values of 5000, 15000, 25000 for the conditioning predictor \vn{income}  produces a simpler graph:
...
The number of points at which a numeric focal predictor is evaluated is controlled by the \ar{focal.levels} argument. The default of \vn{focal.levels=50} is recommended for drawing graphs, but if the goal is to produce a table of fitted values a smaller value such as \code{focal.values=5} produces more compact output.  With the \code{predictorEffects} function the \ar{focal.levels} can be set separately for each focal predictor; see \code{help("predictorEffects")}.    
