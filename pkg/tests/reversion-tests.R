...
if (require(nnet)){
  ...
  reversionTest(df.multinom, eff.multinom)
}
...
if (require(MASS)){
  ...
  reversionTest(df.polr, eff.polr)
}
...
if (require(lme4) && require(nlme)){
   ...
   reversionTest(df.lmer, eff.lmer)
}
...
if (require(lme4)){
  ...
  reversionTest(df.glmer, eff.glmer)
}
...
if (require(lme4) && require(robustlmm)){
    ...
    reversionTest(df.rlmer, eff.rlmer)
}
...
if (require(betareg) && require(lme4)){
   ...
   reversionTest(df.betareg, eff.betareg)
}
...
#   reversionTest(df.poLCA, eff.poLCA)
# }
...
if (require(survey)){
  ...
  reversionTest(df.svyglm, eff.svyglm)
}
...
if(require(nlme) && require(MASS)){
  ...
  reversionTest(df.glmmPQL, eff.glmmPQL)
}
...
#      df.glmmPQL,
#      file="reversion-tests.RData")

# To add to reversion tests first
# load("reversion-tests.RData")
